...
Different distance measures are available for calculating the distance between two plots.  The different measures were studied. Some of the distances work only for permuted data like hamming distance while some are computationally intensive. Most of the distances are calculated on the basis of the raw data.  They typically do not consider the graphical elements present in the plot.  

In this paper, we are interested in calculating the distance between the true plot and the null plot and also between all the null plots in a lineup. A human subject is asked to identify the true plot in the lineup. The response is based on any structure present in the true plot and the finite null plots. The graphical element in the plot can affect the decision. For example, a regression line overlaid on a scatterplot may make it easier to pick a plot with the steepest slope. Similarly, the presence of boxes in a side-by-side boxplot may make it easier to identify the largest vertical difference between the means than when looked at a dot plot. The distance metrics should take into account the presence of these graphical elements.  Five different distance measures were used to identify the different characteristics present in a plot. One the distance metrics is generic in the sense that it uses the raw date to calculate the distances and do not consider any graphical elements present in a plot. Other the distance metrics take the type of data and the presence of graphical elements into consideration. \\

For all of the distance measures below, let $X$ denote the true or real dataset with one or two variables. Let $Y$ denote the null dataset obtained from $X$ using any of the above mentioned null generating mechanism.
...
\begin{figure}[hbtp]
\centering
\includegraphics[scale=0.8]{ave-min-sep.pdf}
\label{sep-dist}
	\vspace{-.1in}
...
\section{Distribution of distance metric} \label{sec:distri}

In classical inference, the sampling distribution of the test statistic is functional. In visual inference, the sampling distribution of the true plot, the test statistic, is conceptually composed of infinite null plots. But a human judge can only observe a finite collection of these null plots as seen in Figure \ref{compare}. A distribution of a distance metric compares to the sampling distribution of the test statistic in classical inference. In classical inference, the sampling distribution is obtained under the null hypothesis and the observed value of the test statistic is compared to this distribution. Similarly, in visual inference, the distances between a large number of null plots are obtained and an empirical distribution of the distance metric is obtained. The distance between the true plot and the null plots is then compared to these distribution.

For a lineup of size $m = 20$, the distances between the true plot and all the ($m$ - 1) null plots are calculated. A mean value is calculated by averaging over these distances. This mean value represents the distance for the true plot. Similarly, the mean distances for the $(m - 1)$ null plots in the lineup are calculated by taking the mean of the distances of the particular null plot and the other $(m - 2)$ null plots. These ($m$ - 1) mean distances represent the distances for the null plots. If the true plot has a stronger structure than the null plots, it can be expected the distance for the true plot will be larger than any of the distances for the null plots. If the true plot have structure of similar strength as the null plots and it is unidentifiable, the distance of the true plot will be similar to the distances for the null plots. 

The empirical distribution of the distance measures is obtained in the following way. A null data is generated from the true dataset using the null generating mechanism. Assuming this null data to be the ``true'' data set, a number of null data sets are obtained from this null data by the null generating mechanism and the distances between these datasets are calculated. For a lineup of size $m$, ($m$ - 2) null datasets are generated from each ``null'' dataset. A mean distance is obtained by averaging over all these distances. This process is repeated a large number of times, say, $N$ where $N$ is a large number of the order $10^3$ or $10^4$. These $N$ mean distances or average distances are obtained and plotted to get the empirical distribution of the distance metric. The mean distances for the true dataset and the null datasets are plotted on the empirical distribution. If the mean distance of the true plot is larger than any of the null plots, the lineup would be regarded as ``easy''. Otherwise, it is a ``difficult'' lineup.

The empirical distribution of the distance based on regression is shown in Figure \ref{dist}. To generate this distribution, $N = 1000$ and $m = 20$ are used. Figure \ref{dist_1} shows the lineup plot for $m = 20$ for testing whether there exists a significant linear relationship between $X_1$ and $X_2$. The 19 null plots are generated by fitting the null model and generating from the null model. Figure \ref{dist_2} shows the empirical distribution of the distance in grey with the mean distances for the true plot (in orange) and the null plots (in black). The true plot is easy to be identified in the lineup (Figure \ref{dist_1}). It can also be seen in Figure \ref{dist_2} as the orange line is extreme compared to the black lines. 
...
\centering 
\begin{tabular}{p{1.5cm}l c  p{2cm} c cc l c p{3.5cm}} 
\hline
 Type of Data & Observed Plot && Null Generating Mechanism & A typical null plot && & Difference && \hspace{0.55cm}($p$, $q$, Max; Min) \\ %[0.5ex] % inserts table %heading 
%\hline
% Linear association & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-data1.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot1.pdf}} \end{center} \end{minipage} &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot1.pdf}} \end{center} \end{minipage} &&           \hspace{0.8cm} small\\
 \hline
 Linear association & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{anscombe-1.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{anscombe-null-1.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{anscombe-nbin-1.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (2, 2, 5.7 ; - 2.5)\\
 \hline
Nonlinear relationship & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{anscombe-2.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{anscombe-null-2.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{anscombe-nbin-2.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (2, 10, 6.2 ; - 0.0)\\
 ...
 \hline
Categorical & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{data2.pdf}} \end{center} \end{minipage} && Simulation from a Normal distribution &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{null2.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot2.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (3, 3, 30.7; 6.2) \\
 \hline
...
\hline
 Type of Data & Observed Plot && Null Generating Mechanism & A typical null plot && & Difference && \hspace{0.55cm}($p$, $q$, Max; Min) \\ %[0.5ex] % inserts table %heading 
 \hline
Nonlinear relation with outliers & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{data3.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{null3.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot3.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (4, 10, 3.9; -3.4) \\
 \hline
Linear relationship with outlier  & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{data5.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{null5.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot5.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (3, 10, 0.3; -7.1) \\
 \hline
 Residual Plot & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{data4.pdf}} \end{center} \end{minipage} && Simulation from the null model &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{null4.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot4.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (3, 7, 17.8; -4.5)\\
 \hline
 Residual Plot & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{data6.pdf}} \end{center} \end{minipage} && Simulation from the null model &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{null6.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot6.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (2, 5, 4.8; -4.4)\\
 \hline
Spiral data & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{data7.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{null7.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot7.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (5, 7, 23.6; -11.9)\\
 \hline
Contami -nated data & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{data8.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{null8.pdf}} \end{center} \end{minipage} &&&  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot8.pdf}} \end{center} \end{minipage} &&           \hspace{0.6cm} (5, 3, 8.1; -2.5)\\
 \hline
%Clusters & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-data2.pdf}} \end{center} \end{minipage} && Permutation &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot2.pdf}} \end{center} \end{minipage} && \hspace{0.8cm} small\\
% \hline
% Discrete vs. Continuous & \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-data3.pdf}} \end{center} \end{minipage} && Simulation from a specific distribution &  \begin{minipage}[h]{1.5cm} \begin{center} \scalebox{0.25}{\includegraphics{bin-select-plot3.pdf}} \end{center} \end{minipage} && \hspace{0.8cm} small\\
...
In this experiment, all the lineups generated had a side-by-side boxplot as the test statistic. The null plots were generated by assuming that there is no difference between the two distributions. The subjects were shown a set of lineups and were asked to identify the plot which has the largest vertical difference between Group A and Group B. Figure \ref{turk1} gives such a lineup.  \\

\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{turk1-example.pdf}
...
The response of the subjects were noted and for each lineup the detection rate was calculated by dividing the number of correct response over the total number of response.  The distances between the plots in each lineup were computed using both the distance based on boxplots ($d_{\hbox{box}}$) and the binned distance ($d_{\hbox{bin}}$). The mean distance for the true plot and the null plots were calculated and $\delta_{\hbox{lineup}}$ and $\gamma_{\hbox{lineup}}$ were obtained. The detection rate was plotted against each of the two statistics. Figure \ref{turk1dist} shows the detection rate against the difference for $d_{\hbox{box}}$ and $d_{\hbox{bin}}$ and the number of null plots greater than the observed plot for the two distance measures. 
...
In Figure \ref{turk1dist}, the detection rate is plotted against the difference. The red vertical line represents difference equal to 0 indicating that the mean distance of the true plot is equal to the maximum of the mean distance of the null plots i.e. the mean distance of the true plot is equal to at least one of the mean distance of the null plots. It can be seen that as the difference increases, the detection rate increases. So the subjects do better in the easier lineups than the hard ones. The binned distance was calculated using 8 bins on the y-axis and 2 bins on the x-axis. Figure \ref{turk1dist} also shows the relation between detection rate and the number of null plots larger than the true plot. It can be seen that as there are more extreme null plots compared to the observed plot, the subjects find it difficult to pick the observed plot. It is interesting to see that the subjects can pick the observed plot with one or two extreme null plots. 

Though the distance based on the boxplots works better, the binned distance does a decent job in this case. Based on the binned distance, there are a few lineups which has a negative difference but the detection rate is above 60\%, which can be also be seen in Figure \ref{turk1dist}. It should be noted that the binned distance does not take into account the graphical elements of the plot (e.g. boxplot) and calculates the distance solely based on the data. So an outlier may have a huge effect on the binned distance but does not effect the distance based on the boxplots. 

The time taken to respond by the subjects is another measure of difficulty of the lineups. The mean time taken by the subjects for each lineup is looked at and plotted against the difference for both the distance measures. Figure \ref{turk1-mtime} shows the plots. It can be clearly seen that when the difference is below 0, there is no real trend in the mean time and there is a huge variability, indicating that the time taken depends on the subjects. But when the difference is above 0, the mean time decreases rapidly as the difference increases. Hence the subjects can pick the true plot quickly if the true plot is extreme compared to the null plots.
...
It can be noticed in Figure \ref{turk1dist} that for some of the lineups, the detection rate is high but the difference using distance metric is negative suggesting that the lineup is difficult. One such lineup is marked using a triangle in Figure \ref{turk1dist}. Investigating the lineup may reveal the reasons behind most human subjects picking the true plot as different from the null plots. But the difference for all the distance metrics is negative. Figure \ref{turk1exp} shows the lineup and the distributions of the distance metrics. 
...
\vspace{-.1in}
\caption[Investigating a lineup with high detection rate but negative difference for the distance metrics]{Plot showing the lineup in (a) and the distribution of different distance metrics : box plot distance in (b), binned distance with 2 and 8 bins on x and y axis in (c) and binned distance with 2 bins in both axes in (d). The lineup corresponds to the point marked with a triangle in difference vs. detection rate plot in Figure \ref{turk1dist}.}
\label{turk1exp}
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\end{figure}

The lineup in Figure \ref{turk1exp} is a lineup of side-by-side boxplots from the experiment. This particular lineup is marked with a triangle in Figure \ref{turk1dist}.  The observed data plot is Plot 20 but there are other null plots which also have strong structures, for example Plot 19 and Plot 16. Specifically in Plot 16, the difference between the first quartiles for the two groups is huge although the differences between the medians and the third quartiles are not so. As the distance based on the boxplots looks at the difference between the quartiles, this huge difference will affect the distance and hence according to this distance, Plot 16 comes out to be the most different. Using the optimal number of bins selection procedure, the number of bins obtained is 2 on x axis and 8 on y axis. Using these bins, the binned distance shows that Plot 20 is the most different.

\subsection{Turk Experiment -- Scatterplots with an Overlaid Regression Line}

In this experiment, the test statistic is a scatterplot with a regression line overlaid. The null hypothesis in this case is that there does not exist a significant linear relationship between $X_1$ and $X_2$. The null plots are generated assuming that the null hypothesis is true. The subjects were shown a set of lineups and were asked to identify the plot which has the steepest slope. Figure \ref{turk2} gives such a lineup. 

\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{turk2-example.pdf}
%\includegraphics[width=.65\textwidth]{large-p-trim-bin.pdf}
...
\includegraphics[scale=0.75]{turk2-mtime-reg-bin.pdf}
	\vspace{-.1in}
\caption[Optional caption for list of figures]{Plot showing the mean time to respond by the subjects against the difference based on the regression distance and binned distance. In both the plots, the vertical line represents the difference equal to 0 when there is at least one null plot similar to the observed plot. The mean time decreases with the difference.  }
\label{turk2-mtime}
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\end{figure}
...
%\subfigure[]{
%\includegraphics[scale=0.47]{distribution-reg-no-int-dist-exp2.pdf}
%\label{t2comp_1}
%}
%\subfigure[]{
%\includegraphics[scale=0.47]{distribution-pval-exp2.pdf}
%\label{t2comp_1}
%}
	\vspace{-.1in}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{largep-example.pdf}
%\includegraphics[width=.65\textwidth]{large-p-trim-bin.pdf}
...
\centering
\subfigure[]{
\includegraphics[scale=0.25]{lineup-example-100-0-2-3.pdf}
\label{t2comp_1}
}
\subfigure[]{
\includegraphics[scale=0.25]{bin-dist-largep-100-0-2-3.pdf}
\label{t2comp_1}
}
\subfigure[]{
\includegraphics[scale=0.25]{mod-sep-dist-largep-100-0-2-3.pdf}
\label{t2comp_2}
}
\subfigure[]{
\includegraphics[scale=0.25]{sep-dist-largep-100-0-2-3.pdf}
\label{t2comp_1}
}
